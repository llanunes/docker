...
Class DL_Mailchimp{
    ...
    public function add_subscribe( $listed_id, $get, $data){
        ...
        if( empty($this->api_key) OR $this->api_key == ''){
			return esc_html__('Please Enter MailChimp API Key.', 'saasland-core');
        }
        
        if(strlen($this->api_key) > 1){

            if(empty(trim($listed_id))){
                return esc_html__('Please select any MailChimp listed.', 'saasland-core');
            }
    ...
    public function get_lists($load = true, $listed = ''){
		...
		if(strlen($this->api_key) > 1){
			...
			try {
				...
				return esc_html__('Api Errors', 'saasland-core');
			}		
		}
		return esc_html__('Enter API Keys', 'saasland-core');
    }
